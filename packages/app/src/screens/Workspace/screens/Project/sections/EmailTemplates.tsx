...
import { useInfiniteListEmailTemplates } from '../hooks'
...
export const EmailTemplates: FC = () => {
  ...
  const { openCreateEmailTemplate } = useModals()
  ...
  return (
    <Box width="100%" paddingBottom="$space700">
      <Stack direction="column" spacing="$space80">
            ...
            <Stack direction="row" spacing="$space40" alignItems="center">
              ...
              <Stack direction="row" spacing="$space40" alignItems="center">
                ...
                <Text size="$size80" color="$gray11">
                  {layoutsTotalCount > 1 ? 'email templates' : 'email template'}
                </Text>
              </Stack>
            </Stack>
          </Stack>
        ...
        <Table
          ...
          primaryAction={{
            ...
            onAction: template =>
              navigate(
                routes.projectEmailTemplateEditor.url({
                  pathParams: {
                    workspaceKey,
          ...
          columns={[
            ...
            },
            {
              ...
              key: 'updatedBy',
              bodyCell: template => (
                <span>{getName(template.updatedBy ?? template.createdBy)}</span>
              ),
            },
            {
              ...
              key: 'updatedAt',
              bodyCell: template => (
                <span>{formatRelative(new Date(template.updatedAt))}</span>
              ),
            },
            {
              headerCell: 'Actions',
                    ...
                    },
                    {
                      ...
                      onSelect: () => {
                        confirmationModalRef.current.confirm().then(res => {
                          ...
                          deleteTemplate({ body: { id: template.id } })
                        })
                      },
                    },
                  ]}
      ...
      <ConfirmationModal
        ...
        description="Once an email template is deleted, its content cannot be recovered."
