...
export const flatten = (node: Node, descend = true) => {
  ...
  const result: Array<{ node: Node; pos: number }> = []
...
export const findChildrenByAttr = (
  node: Node,
  predicate: (node: Node['attrs']) => boolean,
  descend: boolean,
) => {
...
export const findChildrenByType = (
  ...
  return findChildren(node, child => child.type.name === nodeTypeName, descend)
}
...
// }
// ```
export const contains = (node: Node, nodeType: Node['type']) => {
  return !!findChildrenByType(node, nodeType.name, true).length
}
